...
function Ss() {
    if [ -z "$1" ]; then
        echo "Usage: Ss <package-name>"
        return 1
    fi

    echo "\nSearching for '$1' in yay..."
    yay -Ss "$1"
}

function S() {
    if [ -z "$@" ]; then
        echo "Usage: S <package-name> [...<package-name>]"
        return 1
    fi

    echo "\nSearching for '$@' in yay..."
    yay -S "$@" --answerclean All --answerdiff None --answerupgrade None --noconfirm --cleanafter --removemake
}
